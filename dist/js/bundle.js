...
(function($) {
  'use strict';
  $('.fw-parallax .block:first-child .half-lay.left').append("<div class='scro-down floating for-desk'></div> ");
  $(window).on('scroll',function(){
    ...
    // Determine position of .fw-parallax
    if($('.fw-parallax').length > 0){
      var item = $('.fw-parallax'),
          item_top = item.offset().top,
          item_bottom = item_top + item.height();

      // Detect position .fw-parallax
      if(page_top >= item_top && page_range <= item_bottom){
        ...
        for(var i=1; i<=number_item; i++){
          var sub_item = $('.fw-parallax .block:nth-child('+i+') .right'),
  ...
  if (ua.indexOf('safari') != -1) { 
    if (ua.indexOf('chrome') > -1) {
      ...
      $('.fw-parallax .block .half-lay.left').height($(window).height()/2);
      $('.fw-parallax .block:first-child .half-lay.left').height($(window).height());
    }
  }
})(jQuery);
(function($) {
...
(function($) {
  ...
  $('.fw_scrolling_dot .dot-form').each(function(){
    var me = $(this);
...
(function($) {
  ...
  $('.fw-parallax .block').each(function(i){
  ...
  $.fn.fw_get_img = function(options){
    var me = $(this);
    var settings = $.extend({},defaults,options)
...
>>>>>>> 70489022218b970219ddf4797ea8d4b30eb2020a
